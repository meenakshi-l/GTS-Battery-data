...
def eval_historical_average(traffic_reading_df, period):
    ...
    #plt.plot(y_predict.index,y_predict['Voltage(V)'])
    #plt.plot(y_test.index,y_test['Voltage(V)'])
    #plt.savefig('y_predict_test_volt.jpg')
    traffic_reading_df.to_csv('traffic_reading_df_evalhistav.csv')
    y_predict.to_csv('y_predict_evalhistav.csv')
    y_test.to_csv('y_test_evalhistav.csv')
    #print('y_predict ha',y_predict)
    #print('y_test ha', y_test)
    rmse = masked_rmse_np(preds=y_predict.values, labels=y_test.values, null_val=0)
    mape = masked_mape_np(preds=y_predict.values, labels=y_test.values, null_val=0)
    mae = masked_mae_np(preds=y_predict.values, labels=y_test.values, null_val=0)
...
def eval_var(traffic_reading_df, n_lags=3):
    ...
    y_predicts, y_test = var_predict(traffic_reading_df, n_forwards=n_forwards, n_lags=n_lags,
                                     test_ratio=0.2)
    #print('eval_var',y_predicts,y_test)
    #plt.plot(y_predicts['Date_Time'],y_predicts['Voltage(V)'])
    #plt.plot(y_predicts['Date_Time'].index,y_test['Voltage(V)'])
    #plt.savefig('y_predict_test_var.jpg')
    y_test.to_csv('y_test_evalvar.csv')
    #y_predicts.to_csv('y_predicts_evalvar.csv')
